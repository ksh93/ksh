...
[+?\abegin\a\b{\b ... \b}end\b delimit multiline code blocks that override
	...
	blocks should be compatible with the C99 language standard for
	maximal portability. Test code may call the function \bNOTE("...")\b
	to emit short text in \b--verbose\b output; only one \bNOTE()\b
	should be called per test for readability. In addition to all macro
	definitions generated by previous tests, all generated code contains

...
#  define array_fixed_data(ap)	((ap)?((struct fixed_array*)((ap)->fixed))->data:NULL)
...
static Namarr_t *array_scope(Namarr_t *ap, nvflag_t flags)
{
	...
	memset(ar->val, 0, (size_t)ar->maxi*sizeof(char*));
...
static struct index_array *array_grow(Namval_t *np, struct index_array *arp,ssize_t maxi)
{
	...
	memset(ap->bits, 0, (size_t)newsize);
	if(arp)
	{
		ap->header = arp->header;
		ap->header.hdr.dsize = sizeof(*ap) + (size_t)i;
		for(i=0;i < arp->maxi;i++)
		{
			ap->bits[i] = arp->bits[i];
			ap->val[i] = arp->val[i];
		}
		memcpy(ap->bits, arp->bits, (size_t)arp->maxi);
...
static Namarr_t *nv_changearray(Namval_t *np, void *(*fun)(Namval_t*,const char*,nvflag_t))
{
	...
	nv_stack(np,&ap->hdr);
	save_ap = (struct index_array*)nv_stack(np,NULL);
...
Namval_t *nv_arraychild(Namval_t *np, Namval_t *nq, int c)
{
	...
	Namval_t		*tp;
	if(!nq)
		return ap ? array_find(np,ap, ARRAY_LOOKUP) : NULL;
	if(!ap)
	{
...
int nv_nextsub(Namval_t *np)
{
	...
	else if(fp = (struct fixed_array*)ap->header.fixed)
	{
		if(ap->header.nelem&ARRAY_FIXED)
		{
			...
			{
				nv_putsub(np,NULL,fp->curi|ARRAY_FIXED|ARRAY_SCAN);
	...
	for(dot=(size_t)ap->cur+1; dot < (size_t)ap->maxi; dot++)
	{
		aq = ap;
...
static void array_fixed_setdata(Namval_t *np,Namarr_t* ap,struct fixed_array* fp)
{
	...
	long n = ap->nelem;
	ap->nelem = 1;
	fp->size = fp->ptr?sizeof(void*):nv_datasize(np,NULL);
	ap->nelem = n;
...
static int array_fixed_init(Namval_t *np, char *sub, char *cp)
{
	...
	if(*ep)
		return 0;
	sz = (ssize_t)sizeof(struct fixed_array)+ 3*n*(ssize_t)sizeof(ssize_t);

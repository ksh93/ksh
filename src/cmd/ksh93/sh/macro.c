...
int sh_macexpand(struct argnod *argp, struct argnod **arghead,int flag)
{
	...
	if(mp->pattern)
		mp->arrayok = 0;
	/* copyto may call varsub, which may call nv_getval, which may (via other calls) longjmp if a shell
	 * discipline function throws an error, so add this to the longjmp chain to restore global state */
...
void sh_machere(Sfio_t *infile, Sfio_t *outfile, char *string)
{
	...
	while(1)
	{
		...
		s = (cp - 1) - fcseek(0);
		if(s)
			sfwrite(outfile,fcseek(0),(size_t)s);
		cp = fcseek(s + 1);
		switch(n)
		{
		    case S_EOF:
			...
			switch(n=sh_lexstates[ST_DOL][c])
			{
				...
				{
					while(c = fcgetc(), isaname((wchar_t)c))
						sfputc(stkp,c);
					fcseek(-1);
				}
				sfputc(stkp,0);
				...
				varsub(mp);
				write_len = stktell(stkp)-offset2;
				if(write_len > 0)
					sfwrite(outfile,(char*)stkptr(stkp,offset2),(size_t)write_len);
				fcrestore(&save2);
				stkseek(stkp,offset);
				break;
			    }
...
static int varsub(Mac_t *mp)
{
	...
	size_t		replen=0;
	ptrdiff_t	offset = -1;
	ptrdiff_t	ppnum = 0;  /* positional parameter number */
	mp->wasexpan = 1;
retry1:
	idbuff[0] = 0;
	idbuff[1] = 0;
	...
	switch(isascii((int)c)?sh_lexstates[ST_DOL][c]:S_ALP)
	{
		...
		*id = (char)c;
		...
		{
			int d;
			uintmax_t n = (uintmax_t)ppnum;
			while((d=fcget()),isadigit(d))
			{
				n = 10 * n + (uintmax_t)(d - '0');
				if (n > PTRDIFF_MAX)
				{
		...
		{
			sh.used_pos = 1;
			v = getdolarg(ppnum, &vsize);
		}
#endif  /* SHOPT_FILESCAN */
		else if(ppnum <= sh.st.dolc)
		{
			sh.used_pos = 1;
			v = sh.st.dolv[ppnum];
		}
		...
		if(!v && sh_isoption(SH_NOUNSET))
		{
			...
			if(!(d && strchr(":+-?=",d)))
			{
				errormsg(SH_DICT,ERROR_exit(1),e_notset,ltos(ppnum));
				UNREACHABLE();
			}
		}
		break;
		...
		if(type)
		{
			if(mp->dotdot)
			{
				...
				if(ap)
				{
					...
					if(array_assoc(ap))
						arrmax = sh_strdup(v);
					else if((dolmax = (ptrdiff_t)sh_arith(v))<0)
...
retry2:
	if(v && (!nulflg || *v ) && c!='+')
	{
		...
		int index;
		ssize_t nmatch, nmatch_prev;
		ptrdiff_t vsize_last = 0;
		size_t tsize;
		...
		while(1)
		{
			...
			else
			{
				 ...
				 */
				if(mp->pattern && mp->quote && mp->ifs)	/* avoid BUG_IFSGLOBS */
					sfputc(sfio_ptr, '\\');
...
static void comsubst(Mac_t *mp,Shnode_t* t, char type)
{
	...
	else
	{
		...
		sp = sfnew(NULL,str,(size_t)stakoff,-1,SFIO_STRING|SFIO_READ);
		c = sh.inlineno;
	...
	*mp = savemac;
	...
	while((str = sfreserve(sp,SFIO_UNBOUND,0)) && (s = (ptrdiff_t)(bufsize = sfvalue(sp))) > 0)
	{
#if SHOPT_CRNL
		/* eliminate <cr> */
		char *dp;
		char *buff = str;
		while(s > 1 && (*str !='\r' || str[1] != '\n'))
		{
			s--;
			str++;
		}
	...
	else if(mp->pattern>=2 || (mp->pattern && nopat) || mp->assign==3)
	{
		...
		while(size-->0)
		{
			...
			if(c)
			{
				ptrdiff_t d;
				if(d = (cp - 1) - str)
					sfwrite(stkp,str,(size_t)d);
				sfputc(stkp,ESCAPE);
				str = cp-1;
			}
		}
		if(p = cp-str)
			sfwrite(stkp,str,(size_t)p);
	}
 ...
 */
static ssize_t substring(const char *string,size_t len,const char *pat,ssize_t match[], regflags_t flag)
{

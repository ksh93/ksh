...
Namval_t *nv_addnode(Namval_t* np, int remove)
{
	...
	{
		sp->maxnodes += 20;
		sp->nodes = (Namval_t**)sh_realloc(sp->nodes,sizeof(Namval_t*)*(size_t)sp->maxnodes);
	}
...
void nv_setlist(struct argnod *arg,nvflag_t flags, Namval_t *typ)
{
	...
	if(maketype)
	{
		...
		shtp.maxnodes = 20;
		shtp.rp = NULL;
		shtp.nodes = (Namval_t**)sh_malloc((size_t)shtp.maxnodes*sizeof(Namval_t*));
	}
#if SHOPT_NAMESPACE
	...
	if(sh.prefix)
	{
		...
		sh.prefix_root = sh.first_root = NULL;
...
{
	char			*sub=NULL, *cp=(char*)name, *sp, *xp;
	int			c;
	Namval_t		*np=NULL, *nq=NULL;
	Namfun_t		*fp=NULL;
			...
			*sp = 0;
			...
			do
			{
				...
				if(c=='[' || (c=='.' && nv_isarray(np)))
				{
...
#endif /* SHOPT_FIXEDARRAY */
						if((nvflags&NV_ADD)&&(flags&NV_ARRAY))
							nvflags |= ARRAY_FILL;
						if(flags&NV_ASSIGN)
							nvflags |= NV_ADD;
						cp = nv_endsubscript(np,sp,nvflags|(flags&(NV_ASSIGN|NV_FARRAY)));
#if SHOPT_FIXEDARRAY
						flags &= ~NV_FARRAY;
						if(fixed)
							flags &= ~NV_ARRAY;
					...
					else if(c==0 && mode && (idx=nv_aindex(np))>0)
						nv_putsub(np,NULL,idx);
#if SHOPT_FIXEDARRAY
					else if((nvflags==0 || idx==0) && !fixed && (c==0 || (c=='[' && !nv_isarray(np))))
#else
					else if((nvflags==0 || idx==0) && (c==0 || (c=='[' && !nv_isarray(np))))
#endif /* SHOPT_FIXEDARRAY */
					{
						/* subscript must be 0 */
						/* avoid double arithmetic evaluation */
						if(sh.nv_putsub_already_called_sh_arith)
							idx = sh.nv_putsub_idx;
						else
						{
							cp[-1] = 0;
							idx = sh_arith(sp+1);
		...
		if(dtdelete(root,np))
		{
			if(!(flags&NV_NOFREE) && ((flags&NV_FUNCTION) || !nv_subsaved(np,flags&NV_TABLE)))
			{
				...
				if(nv_isarray(np) && np->nvfun && (ap=nv_arrayptr(np)) && array_assoc(ap))
				{
					...
					while(nv_associative(np,NULL,NV_ANEXT))
						nv_associative(np,NULL,NV_ADELETE);
					nv_associative(np,NULL,NV_AFREE);
					free(np->nvfun);
				}
				if(!nv_ispredef(np))
					free(np);
...
Namval_t *nv_open(const char *name, Dt_t *root, nvflag_t flags)
{
	...
	if(root==sh.fun_tree)
	{
		...
		if(strchr(name,'.'))
		{
			name = cp = copystack(NULL,name,NULL);
			fname = strrchr(cp,'.');
			*fname = 0;
...
skip:
	...
	if(c=='=' && np && (flags&NV_ASSIGN))
	{
		...
		{
			char *sub = NULL, *prefix = sh.prefix;
			Namval_t *mp;
			Namarr_t *ap;
			int isref;
			nvflag_t f;
...
#endif /* SHOPT_FIXEDARRAY */
				sub = nv_getsub(np);
			f = msg==e_aliname? 0: (append | (flags&NV_EXPORT));
			if(isref)
				nv_offattr(np,NV_REF);
...
static int scanfilter(Namval_t *np, struct scan *sp)
{
	...
	if(sp->scanmask?(k&sp->scanmask)==sp->scanflags:(!sp->scanflags || (k&sp->scanflags)))
	{
		if(tp && tp->mapname)
		{
			...
			{
				size_t n = strlen(tp->mapname);
				if(strncmp(np->nvname,tp->mapname,n) || np->nvname[n]!='.' || strchr(&np->nvname[n+1],'.'))
					return 0;
			}
			else if((sp->scanflags==NV_UTOL||sp->scanflags==NV_LTOU) && (cp=(char*)nv_mapchar(np,NULL)) && strcmp(cp,tp->mapname))
				return 0;
		}
...
void sh_scope(struct argnod *envlist, int fun)
{
	...
	if(envlist)
	{
		...
		nv_setlist(envlist,NV_EXPORT|NV_NOSCOPE|NV_IDENT|NV_ASSIGN,NULL);
		if(!fun)
			return;
		sh.var_tree = dtview(newscope,NULL);
	}
...
void nv_newattr(Namval_t *np, nvflag_t newatts, ssize_t size)
{
	...
	int trans;
	size_t oldsize;
	Namfun_t *fp= (newatts&NV_NODISC)?np->nvfun:NULL;
	char *prefix = sh.prefix;
	newatts &= ~NV_NODISC;
...
{
	Namval_t	*nq = NULL, *nr = NULL;
	char		*ep, *cp;
	Dt_t		*root = sh.last_root, *hpnext = NULL;
	Namarr_t	*ap = NULL;
	Dt_t		*openmatch;
	struct Namref	*nrp;
	if(nv_isref(np))
...
size_t nv_setsize BYPASS_MACRO (Namval_t *np, size_t size)
{
